...
public class RocksJavaTest {
	...
	private static final String dbPath = "G:\\store\\0";
	...
	public void testCertainColumnFamily() throws RocksDBException {
		...
		for(int i=0; i<10; i++) {
			String k1 = key + i;
			String v1 = value + i;
			rocksDB.put(columnFamilyHandle, k1.getBytes(), v1.getBytes());
		}
		
		RocksIterator iterator = rocksDB.newIterator(columnFamilyHandle);
		for (iterator.seek("certainKey3".getBytes()); iterator.isValid(); iterator.next()) {
