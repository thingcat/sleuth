...
public final class ProtostuffUtil {
	...
	public static <T extends Message<T>> T copy(T t) {
		try {
			...
			throw new RuntimeException("Failure decoding object, please ensure that you ran dremio-admin upgrade on Dremio.");
		}
	}
	 ...
	 *            object to update
	 * @param schema
	 *            the Protostuff schema for the object
	 * @param numeric
	 *            if true, use field id as key
	 */
	public static <T> void fromJSON(byte[] data, T message, Schema<T> schema, boolean numeric) throws IOException {
		...
		try (JsonParser parser = JsonIOUtil.newJsonParser(null, data, 0, data.length)
	 ...
	 *            message to write
	 * @param schema
	 *            the protostuff schema for the message
	 * @param numeric
	 *            if true, use field id as keys
	 */
	public static <T> void toJSON(OutputStream out, T message, Schema<T> schema, boolean numeric) throws IOException {
		try (JsonGenerator jsonGenerator = JsonIOUtil.DEFAULT_JSON_FACTORY.createGenerator(out, JsonEncoding.UTF8)
