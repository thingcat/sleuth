...
public final class Base58Check {
	...
	public static final int ADDRESS_CHECKSUM_LEN = 4;// 校验码长度
    ...
    static byte[] addCheckHash(byte[] data) {
        try {
            byte[] hash = Arrays.copyOf(PublicKeyUtils.doubleHash(data), ADDRESS_CHECKSUM_LEN);
    ...
    public static byte[] base58ToBytes(String s) {
        ...
        byte[] data = Arrays.copyOf(concat, concat.length - ADDRESS_CHECKSUM_LEN);
        byte[] hash = Arrays.copyOfRange(concat, concat.length - ADDRESS_CHECKSUM_LEN, concat.length);
        byte[] rehash = Arrays.copyOf(PublicKeyUtils.doubleHash(data), ADDRESS_CHECKSUM_LEN);
