 ...
 * @date 2020年10月17日
 */
@Component("clientSubscribeHandle")
public class ClientSubscribeHandle implements MessageHandle<SubProtocol> {
	...
	public void handle(MessageManager manager, Channel channel, SubProtocol message) {
		...
		if (message.getStage() == 1) {
			logger.info("Subscription successful, ch = {}", message.getCh());
		} else {
			logger.info("Subscription unsuccessful, ch = {}", message.getCh());
		}
	}
